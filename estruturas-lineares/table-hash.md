...
- **Overhead de Ponteiros e Objetos**: Implementações comuns de tabelas hash (como HashMap em Java) armazenam pares chave-valor como objetos separados, que, por sua vez, têm sua própria sobrecarga de memória (ponteiros, cabeçalhos de objetos, etc.).
- **Fator de Carga (Load Factor)**: Para manter um bom desempenho (tempo de busca rápido), as tabelas hash alocam mais espaço do que o estritamente necessário para os itens armazenados no momento. Se o fator de carga for muito baixo, uma parte significativa da tabela fica vazia, desperdiçando memória. 
